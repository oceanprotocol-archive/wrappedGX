...
contract('wrappedGX', (accounts) => {
  ...
  describe('Functions with a positive balance', async () => {
    ...
    it('Alice transfers WGX to address(0)', async () => {
      const balanceBefore = await WGX.balanceOf(alice)
      await WGX.transfer('0x0000000000000000000000000000000000000000', 1, {
        from: alice
      })
      const balanceAfter = await WGX.balanceOf(alice)
      balanceAfter
    ...
    it('Alice and Bob cannot transfer beyond their balance', async () => {
      ...
      await expectRevert(
        WGX.transferFrom(alice, bob, 100, { from: alice }),
        'revert'
      )
      await expectRevert(WGX.transfer(alice, 100, { from: bob }), 'revert')
      await expectRevert(
        WGX.transferFrom(bob, alice, 100, { from: bob }),
        'revert'
      )
    })
    it('Alice approves to increase Bob`s allowance', async () => {
      ...
      allowanceAfter
        .toString()
        .should.equal(allowanceBefore.add(new BN('1')).toString())
    })
  })
  ...
  describe('Functions with a positive allowance', async () => {
    ...
    it('Alice and Bob cannot transfer beyond allowance', async () => {
      await expectRevert(
        WGX.transferFrom(alice, bob, 2, { from: bob }),
        'revert'
      )
      await expectRevert(
        WGX.transferFrom(bob, alice, 2, { from: alice }),
        'revert'
      )
    })
  })
